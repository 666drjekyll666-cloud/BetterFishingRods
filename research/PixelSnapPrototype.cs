...
namespace BiteCountdownPixelSnapResearch
{
    [BepInPlugin(PluginGuid, PluginName, PluginVersion)]
    public sealed class Plugin : BaseUnityPlugin
    {
        public const string PluginGuid = "nikich.bitecountdown.pixelsnapprototype";
        public const string PluginName = "Bite Countdown Pixel Snap Prototype";
        ...
        private PixelSnapMode _pixelSnapMode = PixelSnapMode.Current;
        private Transform _activeBobber;
        private Vector2 _baseAnchorLocal;
        private string _lastSnapDiagnostic = "No active ring yet.";
        ...
        private void StartRing(object fishingGui)
        {
            ...
            _baseAnchorLocal = anchor + acceptedOffset;
            ...
            int generation)
        {
                ...
                && IsState(fishingGui, "WaitingForBite"))
            {
                ...
                if (Input.GetKeyDown(KeyCode.F1))
                    SetPixelSnapMode(PixelSnapMode.Current);
                if (Input.GetKeyDown(KeyCode.F2))
                    SetPixelSnapMode(PixelSnapMode.IntegerPixel);
                if (Input.GetKeyDown(KeyCode.F3))
                    SetPixelSnapMode(PixelSnapMode.HalfPixel);

                ApplyPixelSnapPosition();
            ...
            Vector2 finalAnchor)
        {
            if (_ringObject == null)
            {
                _ringObject = new GameObject(
                    "BiteCountdownPixelSnap_CountdownRing");
        ...
        private void SetPixelSnapMode(PixelSnapMode mode)
        {
            _pixelSnapMode = mode;
            ApplyPixelSnapPosition();
            Logger.LogInfo("PIXEL_SNAP_MODE mode=" + mode);
        }

        private void ApplyPixelSnapPosition()
        {
            if (_ringObject == null || _activeBobber == null)
                return;

            if (_pixelSnapMode == PixelSnapMode.Current)
            {
                _ringObject.transform.localPosition =
                    new Vector3(_baseAnchorLocal.x, _baseAnchorLocal.y, 0f);
                return;
            }
            ...
            var baseWorld = _activeBobber.TransformPoint(
                new Vector3(_baseAnchorLocal.x, _baseAnchorLocal.y, 0f));
            var screen = camera.WorldToScreenPoint(baseWorld);

            var snappedX = _pixelSnapMode == PixelSnapMode.IntegerPixel
                ? Mathf.Round(screen.x)
                : Mathf.Round(screen.x - 0.5f) + 0.5f;

            var snappedScreen = new Vector3(snappedX, screen.y, screen.z);
            var snappedWorld = camera.ScreenToWorldPoint(snappedScreen);
            var snappedLocal = _activeBobber.InverseTransformPoint(snappedWorld);

            _ringObject.transform.localPosition =
                new Vector3(snappedLocal.x, snappedLocal.y, 0f);
        }

        private void LogSnapCandidates()
        {
            if (_activeBobber == null)
                return;

            var camera = Camera.main;
            if (camera == null)
            {
                _lastSnapDiagnostic = "Camera.main unavailable.";
                Logger.LogInfo("PIXEL_SNAP_CANDIDATES camera=null");
                return;
            }

            var world = _activeBobber.TransformPoint(
                new Vector3(_baseAnchorLocal.x, _baseAnchorLocal.y, 0f));
            var screen = camera.WorldToScreenPoint(world);
            var integerX = Mathf.Round(screen.x);
            var halfX = Mathf.Round(screen.x - 0.5f) + 0.5f;

            _lastSnapDiagnostic =
                "baseX=" + screen.x.ToString("F3")
                + " integer=" + integerX.ToString("F1")
                + " half=" + halfX.ToString("F1");

            Logger.LogInfo(
                "PIXEL_SNAP_CANDIDATES"
                + " base_x=" + screen.x.ToString("F3")
                + " integer_x=" + integerX.ToString("F1")
                + " integer_delta=" + (integerX - screen.x).ToString("F3")
                + " half_x=" + halfX.ToString("F1")
                + " half_delta=" + (halfX - screen.x).ToString("F3"));
        }

        private void OnGUI()
        {
            GUI.Label(
                new Rect(16f, 16f, 700f, 52f),
                "Bite Countdown pixel-snap test: "
                + _pixelSnapMode
                + "   F1=current  F2=integer  F3=half-pixel\n"
                + _lastSnapDiagnostic);
        }
        ...
        private static Texture2D CreateRingTexture()
        {
            ...
            texture.name =
                "BiteCountdownPixelSnap_CountdownRing_Texture";
        ...
        private void Fail(string stage, Exception ex)
        {
            ...
            Logger.LogError(
                "Bite Countdown Pixel Snap Prototype disabled its countdown after "
                + stage
                + " failure: "
                + ex.GetType().Name
                + ": "
